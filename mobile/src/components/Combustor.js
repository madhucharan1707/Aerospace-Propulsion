...
export class Combustor extends THREE.Group {
  ...
  initEffects(midRadius, linerLen, baseZ) {
    ...
    // INLINED SHADER to avoid import errors
    const vertexShader = `
    varying vec2 vUv;
    ...
    void main() {
        ...
        n += noise(uv * 20.0) * 0.5;
        n += noise(uv * 5.0 + vec2(uTime, 0.0)) * 0.25; 
        n /= 1.75;
        ...
        intensity *= (uThrottle + 0.5); 

        vec3 colorRich = vec3(1.0, 0.3, 0.0);
        vec3 colorLean = vec3(0.0, 0.5, 1.0);
        vec3 colorHot = vec3(1.0, 0.9, 0.5);
        ...
        gl_FragColor = vec4(finalColor * 2.0, alpha * 0.8);
    }
    `;

    // DEFENSIVE CHECK
    if (typeof vertexShader !== 'string' || typeof fragmentShader !== 'string') {
      console.error("CRITICAL: Combustor Shader Strings are invalid!");
      return; // Abort flame creation
    }
